...
$subs = mysqli_query($conn, "SELECT s.*, p.name as plan_name, p.price FROM subscriptions s JOIN plans p ON s.plan_id = p.id ORDER BY s.id DESC");
?>
<!DOCTYPE html>
<html lang="en">
<head>
    ...
    <style>
        :root { --bg: #0a0a0a; --surface: #111111; --card: #161616; --border: #222222; --accent: #ff4500; --accent2: #ff7300; --text: #f0f0f0; --muted: #666; --success: #22c55e; }
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .status-active { background: rgba(34,197,94,0.15); color: var(--success); padding: 3px 10px; border-radius: 20px; font-size: 12px; font-weight: 600; }
        .status-expired { background: rgba(239,68,68,0.15); color: #ef4444; padding: 3px 10px; border-radius: 20px; font-size: 12px; font-weight: 600; }
...
<aside class="sidebar">
    ...
    <nav class="nav">
        ...
        <a href="admin_subscriptions.php" class="active"><span class="icon">📋</span> Subscriptions</a>
...
<main class="main">
    <div class="page-header">
        <h2>Subscriptions</h2>
        <p>All member plan subscriptions</p>
    </div>
    <div class="section">
        <div class="section-header">
            <h3>All Subscriptions</h3>
            <span class="badge"><?php echo mysqli_num_rows($subs); ?> Total</span>
        </div>
        <?php if (mysqli_num_rows($subs) > 0): ?>
        <table>
            <tr><th>ID</th><th>Member ID</th><th>Plan</th><th>Price</th><th>Start Date</th><th>End Date</th><th>Status</th><th>Action</th></tr>
            ...
            <tr>
                ...
                <td><a href="?delete=<?php echo $s['id']; ?>" class="delete-btn" onclick="return confirm('Delete?')">Delete</a></td>
            </tr>
            <?php endwhile; ?>
        </table>
        <?php else: ?>
